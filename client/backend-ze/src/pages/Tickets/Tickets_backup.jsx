                ...
                <div className="ticket-search pb-4">
                    <div className="input-group">
                        <select className="custom-select" id="ticketSearchSelect"
                            ...
                            <option value="ticketId">票卷編號</option>
                ...
                <div className="ticket-display mr-4">
                    <table className="table table-striped table-hover table-bordered col-12">
                        ...
                        <tbody>
                            ...
                            {ticketTableDisplay.filter((val) => {
                                if (ticketSearchbar == "") {
                                    return val;
                                } else if (ticketSearchbarCat == "ticketId" && val.ticketNo.includes(ticketSearchbar)) {
                                    return val;
                                ...
                                return <tr key={key}>
                                    ...
                                    <td className="align-middle"><span>{val.ticketNo}</span></td>
                                    <td className="align-middle"><span>{val.mName}</span></td>
                                    <td className="align-middle"><span>{val.mPhone}</span></td>

                                    <td><span>{val.ticketName}</span></td>
                                    <td><span>{val.campId}</span></td>

                                    {/* {val.tickets.map((val) => {
                                        return <tr><td><span>{val.ticketType}</span></td></tr>
                                    })} */}


                                    {/* <td className="my-table-flex">{val.tickets.map((val) => {
                                        return <span>{val.ticketType}</span>
                                    })}</td>
                                    <ul className="" style={{listStyleType: "none"}}>{val.tickets.map((val) => {
                                        return <li>{val.ticketType}</li>
                                    })}</ul> */}



                                    {/* <td>{val.tickets.map((val) => {
                                        return <tr><td><span>{val.campId == null ? "無" : val.campId}</span></td></tr>
                                    })}</td> */}
